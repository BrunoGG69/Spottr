...
export default function StatCard({label, value, sublabel, accent = 'cyan'}) {
    ...
    return (
        <div className="stats bg-white/3 border border-white/8 w-full">
            <div className="stat py-3 px-4">
                <div className="stat-title text-white/40 text-xs">{label}</div>
                <div className="stat-value text-white text-xl">{value}</div>
                {sublabel && (
                    <div className={`stat-desc text-xs ${accentClasses[accent]}`}>{sublabel}</div>
                )}
            </div>
        </div>
    )
